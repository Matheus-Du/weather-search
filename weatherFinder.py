...
def getTemperature(*args):
    ...
    province = prvStr.get()
    city = cityStr.get()
    location.set("The current weather in {}, {} is: ".format(city, province))
...
window = ttk.Frame(root, padding='3 4 12 40')
...
title = ttk.Label(window, text="Weather Lookup V 0.1")
...
cityEntry.grid(column=1, row=2, sticky=(W, S), columnspan=2)
calcButton = ttk.Button(window, text='Get Weather', command=getTemperature)
calcButton.grid(column=0, row=3, sticky=W)

locationLabel = ttk.Label(window, textvariable=location)
locationLabel.grid(column=1, row=4, sticky=E)
locationLabel.grid_forget()
weatherLabel = ttk.Label(window, textvariable=weather)
weatherLabel.grid(column=2, row=4, sticky=(W, S))
